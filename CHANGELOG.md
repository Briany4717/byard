...
- **Durable key/value storage (RFC-0029 §6).** `Store` persists any value to one
  readable JSON file in the platform data directory, written atomically
  (temp-file + rename in the same directory) so a crash mid-write leaves the old
  file or the new one. A corrupt file loads empty with the failure reported, and
  the next write recovers. `store.get(key, default)` answers the missing key
  every app hits on its first run.
