...
pub fn check_program_with_theme(
    ...
    // (`UncheckableInject`). The *reserved* list is used rather than a built
    // registry, because building one would resolve a data directory and open a
    // store, and a checker must not touch the filesystem an app owns.
    let capabilities: Vec<&str> = byard_core::cap::RESERVED_NAMES.to_vec();
