...
pub enum CompileError {
    ...
    /// A controller call failed and its call site wrote no `err` arm
    /// (RFC-0028 §4).
    ///
    /// Distinct from [`UnknownControllerMethod`](Self::UnknownControllerMethod)
    /// on purpose: "this controller has no such method" is a wiring mistake the
    /// developer fixes in the call, while this is the controller's own runtime
    /// failure, reported because a success nobody asked about is fine and an
    /// **error** nobody asked about is a silent failure.
    ControllerCallFailed {
        /// Source range of the call.
        span: Span,
        /// The controller's own message, taken from the error record.
        message: String,
    },
...
impl CompileError {
    ...
    pub fn headline(&self) -> String {
        match self {
            ...
            Self::ControllerCallFailed { message, .. } => {
                format!("this controller call failed and has no `err` arm: {message}")
            }
