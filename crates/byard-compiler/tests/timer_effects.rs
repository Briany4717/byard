...
"#,
    ...
    harness.drain();
    assert_eq!(harness.text(), "0");
    // And it leaves nothing behind. A continuation opened for a timer that was
    // never armed can never be resumed, so it would sit in the table for as
    // long as the scope stays mounted.
    assert_eq!(
        harness.interp.outstanding_continuations(),
        0,
        "a refused timer must not leave a continuation behind"
    );
}

#[test]
fn an_armed_timer_holds_exactly_one_continuation_for_as_long_as_it_runs() {
    // The counterpart to the refused case: the bookkeeping has to be right in
    // both directions, or `outstanding_continuations` means nothing.
    let mut harness = Harness::new(
        r#"
View Main() {
    var ticks: Int = 0
...
"#,
    ...
    assert_eq!(
        harness.interp.outstanding_continuations(),
        1,
        "a repeating timer resumes into the same continuation, it does not accumulate"
    );
}
