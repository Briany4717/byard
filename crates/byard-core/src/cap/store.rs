...
/// Monotonic per-write counter, so two writers in one process never pick the
/// same temporary name even when they are two independent [`Store`] values
/// over one path (which the per-`Store` lock does not cover).
...
///
/// The temporary name is **unique per write**, not a fixed `.tmp` sibling. The
/// async `Mutex` serialises writers inside one `Store`; it does nothing about
/// two processes running the same app, or two `Store::at` values over one path.
/// Sharing a temporary name across those reintroduces exactly the race the
/// rename exists to prevent: both write the same file, the first rename moves
/// it away, and the second fails with `ENOENT` having lost its write. Process
/// id plus a counter is unique across both axes, and the file is removed on
/// the failure path so a crashed write leaves no litter.
fn store_atomically(path: &Path, map: &BTreeMap<String, HostValue>) -> std::io::Result<()> {
    ...
    let temp = path.with_file_name(format!("{name}.{}.{seq}.tmp", std::process::id()));

    std::fs::write(&temp, text)?;
    match std::fs::rename(&temp, path) {
        Ok(()) => Ok(()),
        Err(e) => {
            // The rename is the only step that can fail after the bytes are
            // down. Leaving the temporary file behind would accumulate one per
            // failed write, in the directory the app's user can see.
...
mod tests {
    ...
    #[test]
    fn two_independent_stores_over_one_file_do_not_collide() {
        // The per-`Store` lock does not cover this: two `Store::at` values are
        // two locks. It stands in for the case that cannot be tested in one
        // process at all, two `byard dev` runs of the same app, which is the
        // one a fixed temporary name would break.
        let temp = TempStore::new("two-stores");
        let a = temp.store.clone();
        let b = temp.reopen();
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .worker_threads(4)
            .enable_all()
        ...
        runtime.block_on(async {
            ...
            for (i, store) in [a, b].into_iter().enumerate() {
                for j in 0..4 {
                    ...
                    tasks.push(tokio::spawn(async move {
                        store
                            .invoke(
                                "set",
                                vec![HostValue::Str(format!("s{i}k{j}")), HostValue::Int(1)],
                            )
                            .await
                    }));
                }
            ...
            for task in tasks {
                // No write may fail: a shared temporary name shows up here as
                // an `ENOENT` on the rename.
