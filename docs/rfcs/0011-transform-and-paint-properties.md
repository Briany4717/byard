...
- **Status:** Active, partially implemented. M33 engine primitives, M34 attribute surface, and group-transform inheritance landed first. The **hierarchical transform stack** landed 2026-09-23: hit testing and clipping now follow an ancestor's transform the way paint already did (below). Group opacity (render-to-texture) landed the same day. Remaining: text transforms, which are blocked on `glyphon` taking no per-run matrix.
  ...
  **Group opacity (T4) is implemented.** A translucent container with children is faded as one picture: its subtree is drawn opaque into an offscreen target the size of the frame, and a single composite puts it back at the container's effective opacity and at a draw-order depth reserved before the group's first primitive, so the picture lands between what was drawn before it and what is drawn after. The visible difference from per-instance opacity is overlap, which is nearly always present, since a card's text sits on the card's own background. Decisions taken while building it:

  - **Automatic, not opt-in**, but only where it can matter: a translucent box *with children*. A translucent leaf has nothing to overlap and keeps the per-instance path, and an opaque box opens nothing; both are asserted, and the second was watched failing against a build that grouped every container.
  - **One level.** A group inside an open group falls back to per-instance within its parent's picture, which is exact whenever the inner content does not overlap itself. Nesting offscreen targets would cost a texture per depth for a case no example has asked for.
  - **One target per frame**, reused by every group. The segmentation puts a frame pass between a group's last piece and the next group's first, which is where the first is composited, so its picture is always consumed before the target is cleared again. An application with no translucent container never allocates it.
