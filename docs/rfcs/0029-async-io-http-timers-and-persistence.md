...
(`directories`-crate resolved path, keyed on a **stable app identity**, the
manifest project name under `byard dev` and the executable name under
`byard::App`, never the entry file's stem, which is `main` for almost every app,
erratum §7), loaded into an in-memory `HashMap<String,
HostValue>` at first access, written atomically on `set`/`remove` to a temporary file whose
name carries the process id and a per-write counter, then renamed: a fixed
temporary name is only atomic within one process (erratum §6). All disk work runs via `spawn_blocking` on the pool. Concurrent `set`s
...
build. Reserved is not the same as **provided**, `Timer` has no controller at
all and a feature may be off, so `cap::provided_names()` answers the second
question without instantiating anything (erratum §8). `App::provide` **rejects** a controller taking one and `run()` fails
