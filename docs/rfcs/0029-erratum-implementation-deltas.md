...
- **Applies to:** RFC-0029 §2 (what the idle gate has to carry), §3 (the
  ...
  reserved name and a provided one), and RFC-0028 §9's treatment of an `inject`
  a static check cannot resolve.
...
## 6. The temporary file cannot have a fixed name (§6)

§6 says the write is a "temp-file + rename". A fixed sibling (`store.json.tmp`)
makes that true only *within one process*: the async `Mutex` serialises writers
inside one `Store`, and does nothing about two processes running the same app,
or two `Store::at` values over one path. Sharing the name reintroduces the very
race the rename exists to prevent, both write the same file, the first rename
moves it away, and the second fails with `ENOENT` having lost its write.

**Correction.** The temporary name carries the process id and a per-write
counter, which is unique across both axes, and the file is removed on the
failure path so a crashed write leaves no litter.
`two_independent_stores_over_one_file_do_not_collide` stands in for the case
that cannot be written in one process at all.

## 7. A store is keyed on the app, not on the entry file (§6)

§6 says the store lives in "the platform data dir", resolved by `directories`,
and never says what identity it is filed under. The first implementation used
the entry file's stem. Almost every app's entry is `src/main.byd`, so that
files **every shipped Byard app's store in one directory called `main`**, and
two unrelated apps share their settings.

**Correction.** `byard::App` keys the store on the executable's own name, which
is as stable as the binary, and exposes `App::app_id(…)` for an app whose binary
is renamed or shipped under several names. It is kept separate from the window
title on purpose: a title is presentation and may be translated, and rewording
one must not move the other. `byard dev` keys on the manifest's project name,
for the same reason it should not be the path.

## 8. A reserved name is not a provided one (§7, §8)

§7 reserves `Http`, `Json`, `Store` and `Timer`. §8's feature table decides
which of them a given build actually *provides*, and the two lists are not the
same: `Timer` is reserved because `every`/`after` own the word and has no
controller behind it at all, and `--no-default-features` reserves every name
while providing none.

Anything that reasons about a name has to ask the right question. `byard check`
asking the reserved list would call `inject Timer as t` fully checkable and let
a runtime failure through.

**Correction.** `cap::provided_names()` answers "what does this build provide",
