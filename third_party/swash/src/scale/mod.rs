...
impl<'a> ScalerBuilder<'a> {
    fn new(
        ...
        // Byard: the coordinate buffer is shared by every scaler this context
        // builds, and `variations` only resizes it, so without this the next
        // font inherited the previous one's coordinates on every axis it did
        // not set itself. See third_party/swash/BYARD.md.
